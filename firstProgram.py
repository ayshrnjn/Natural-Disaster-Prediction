import pandas as pd  # to import the data set

import numpy as np # to do mathematcal calculation
from sklearn.model_selection import train_test_split # to split data set into test and train
...
from sklearn.preprocessing import StandardScaler #data cleaning and processing
from imblearn.over_sampling import SMOTE # data cleaning and sampling
import joblib # for deployment to create pkl file
import streamlit as st # for deployment to make server live


def generate_dataset(num_samples=5000):
    np.random.seed(42)
    temperature = np.random.uniform(15, 55, num_samples)
...
def deploy_with_streamlit():
    ...
    temperature = st.sidebar.slider("Temperature (°C)", 15.0, 55.0, value=25.0, step=0.1)
